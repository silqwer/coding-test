...
function solution(N, stages) {
    ...
    const orderStages = [...stages].sort((a, b) => a - b);
    const failureRates = [];
    
    for(let i=1; i<=N; ++i){
        const stage = i;
        const countOfChallenger = stages.reduce((acc, curr) => {
            if(curr === stage){
                acc += 1;
            }
            return acc;
        ...
        const countOfPasser =  stages.reduce((acc, curr) => {
            if(curr >= stage){
                acc += 1;
            }
            return acc;
        ...
        const failRate = countOfChallenger/countOfPasser;
        failureRates.push([stage, failRate])
    }
    ...
    answer = failureRates.map(item => item[0]);
    return answer;
}
```