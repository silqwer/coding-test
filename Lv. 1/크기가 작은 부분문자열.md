...
```javascript
function countNumberInArr(t, p) {
    const pLength = p.length;
    const tLength= t.length - pLength;
    const arr = [];
    let result = 0;
    ...
    result =  arr.reduce((count, currentValue) => {
        if(currentValue <= Number(p)){
            count += 1;
        }
        return count;
    }, 0);
...
function solution(t, p) {
    let answer = countNumberInArr(t, p); 
    return answer;
}
```